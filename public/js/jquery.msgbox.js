...
(function($, undefined){
	...
	if ($(window).width()>736){
		// scrleft= $(".col-lg-4").width()+'px';
		// scrleft= $(window).width()/3 + $(window).width()/12;
		scrleft=$(".jMsgbox-wrap").width();
		mywidth= 'min-content';
		// mywidth=$(".jMsgbox-content").width();
		// mywidth=$(".jMsgbox-wrap").width($(".col-lg-4").width());

			// setTimeout(function(){ 		
			// 	console.log($(".jMsgbox-wrap").width());
	...
	var DEFAULTS = {
		...
		width: mywidth,			// the width of msgbox
		height: 550,		// the height of msgbox
		initialWidth: mywidth,	// the initial width of msgbox, when loaded, it will be animated to options.width and options.height
		initialHeight: 220, // see initialHeight
		top: '60px',			// the position of msgbox, if false, will show in the middle of the window
		left: scrleft,
		...
		opacity: .8,		// the opacity of the overlay, you can use !important to override this in css
		zIndex: 1500,			// the initial z-index of overlay and wrap. Just in case some other elements in document have greater z-index
		...
		padding: 20,			// the padding of the content
	...
	MSGBOX.prototype = {
		...
		_append: function () {
			if (this.options.overlay) $("#mypopid").append(this.$overlay);
			$("#mypopid").append(this.$wrap.append(this.$title.append(this.$controls), this.$content.append(this.$loaded)));
			if (this.options.buttons !== null && this.options.buttons.length > 0) {
